...
data class BitchatMessage(
    ...
    // Photo support fields
    val isPhoto: Boolean = false,
    val photoData: String? = null, // Base64 encoded image data
    ...
    fun toBinaryPayload(): ByteArray? {
        try {
            ...
            // Photo messages use existing flags - no separate photo flag needed
            ...
            // Content (or photo fallback text if photo message)
            val contentToSend = if (isPhoto && photoData != null) {
                "📷 Photo: ${photoFileName ?: "image"}" // Fallback text for photo
            } else {
                content
            }
            ...
            // Photo data if this is a photo message
            if (isPhoto && photoData != null) {
                val photoBytes = photoData.toByteArray(Charsets.UTF_8)
                val length = minOf(photoBytes.size, 32768) // 32KB limit
                buffer.putInt(length)
                buffer.put(photoBytes.take(length).toByteArray())
                ...
                photoMimeType?.let { mimeType ->
                    val mimeTypeBytes = mimeType.toByteArray(Charsets.UTF_8)
                    buffer.put(minOf(mimeTypeBytes.size, 255).toByte())
                    buffer.put(mimeTypeBytes.take(255).toByteArray())
                } ?: buffer.put(0.toByte())
            }
    ...
    companion object {
        ...
        fun fromBinaryPayload(data: ByteArray): BitchatMessage? {
            try {
                ...
                // Photo messages detected by content pattern
                val isPhoto = false // Will be determined by content parsing
                ...
                if (isPhoto && buffer.hasRemaining()) {
                    // Photo data
                    val photoDataLength = buffer.getInt()
                    if (buffer.remaining() >= photoDataLength) {
                        val photoBytes = ByteArray(photoDataLength)
                        buffer.get(photoBytes)
                        photoData = String(photoBytes, Charsets.UTF_8)
                    }
                    ...
                    if (buffer.hasRemaining()) {
                        ...
                        if (fileNameLength > 0 && buffer.remaining() >= fileNameLength) {
                            ...
                            photoFileName = String(fileNameBytes, Charsets.UTF_8)
                        }
                    }

                    // Photo mime type
                    if (buffer.hasRemaining()) {
                        val mimeTypeLength = buffer.get().toInt() and 0xFF
                        if (mimeTypeLength > 0 && buffer.remaining() >= mimeTypeLength) {
                            val mimeTypeBytes = ByteArray(mimeTypeLength)
                            buffer.get(mimeTypeBytes)
                            photoMimeType = String(mimeTypeBytes, Charsets.UTF_8)
                        }
                    }
                }
    ...
    override fun hashCode(): Int {
        ...
        result = 31 * result + isPhoto.hashCode()
        result = 31 * result + (photoData?.hashCode() ?: 0)
        result = 31 * result + (photoFileName?.hashCode() ?: 0)
        result = 31 * result + (photoMimeType?.hashCode() ?: 0)
        return result
    }
}
