...
fun formatMessageAsAnnotatedString(
    ...
    if (message.sender != "system") {
        ...
        // Message content - handle photo messages differently
        if (message.isPhoto) {
            if (message.photoData != null) {
                // Sender or receiver with photo data - show photo thumbnail
                val photoText = "📷 ${message.photoFileName ?: "Photo"}"
                val startIndex = builder.length
                
                builder.pushStyle(SpanStyle(
                    color = Color(0xFF34C759), // Green for available photo
                    fontSize = BASE_FONT_SIZE.sp,
                    fontWeight = FontWeight.Medium
                ))
                builder.append(photoText)
                val endIndex = builder.length
                builder.pop()
                
                // Add click annotation to view photo
                builder.addStringAnnotation(
                    tag = "photo_view",
                    annotation = message.photoData,
                    start = startIndex,
                    end = endIndex
                )
                ...
                builder.addStringAnnotation(
                    tag = "photo_filename",
                    annotation = message.photoFileName ?: "photo.jpg",
                    start = startIndex,
                    end = endIndex
                )
            } else {
                // Receiver without photo data - show download option  
                val photoText = "📷 ${message.photoFileName ?: "Photo"} (tap to request)"
                val startIndex = builder.length
                
                builder.pushStyle(SpanStyle(
                    color = Color(0xFF007AFF), // Blue for download
                    fontSize = BASE_FONT_SIZE.sp,
                    fontWeight = FontWeight.Medium,
        ...
        } else if (message.content.startsWith("📷 Photo:") && !message.isPhoto) {
            // Handle legacy photo messages (received as text)
            val photoText = "${message.content} (tap to request)"
            val startIndex = builder.length
            ...
            builder.addStringAnnotation(
                tag = "photo_request",
                annotation = message.content.substringAfter("📷 Photo: "),
                start = startIndex,
                end = endIndex
            )
        } else {
            // Regular message content with iOS-style hashtag and mention highlighting
