...
class ChatViewModel(
    ...
    // NEW: Photo message sending
    fun sendPhotoMessage(base64Data: String, fileName: String, mimeType: String) {
        if (base64Data.isEmpty()) return
        
        val selectedPeer = state.getSelectedPrivateChatPeerValue()
        val currentChannelValue = state.getCurrentChannelValue()
        val photoFallbackText = "📷 Photo: $fileName"
        
        // Create photo message
        val photoMessage = BitchatMessage(
            sender = state.getNicknameValue() ?: meshService.myPeerID,
            content = photoFallbackText, // Fallback text for non-photo clients
            timestamp = Date(),
            isRelay = false,
            senderPeerID = meshService.myPeerID,
            channel = currentChannelValue,
        ...
        if (selectedPeer != null) {
            // Send private photo message
            messageManager.addPrivateMessage(selectedPeer, photoMessage)
            
            // Route via MessageRouter (mesh when connected+established, else Nostr)
            val router = com.bitchat.android.services.MessageRouter.getInstance(getApplication(), meshService)
            router.sendPrivate(photoFallbackText, selectedPeer, meshService.getPeerNicknames()[selectedPeer] ?: selectedPeer, photoMessage.id)
            
        } else {
            // Check if we're in a location channel
            val selectedLocationChannel = state.selectedLocationChannel.value
            if (selectedLocationChannel is com.bitchat.android.geohash.ChannelID.Location) {
                // Send photo to geohash channel via Nostr (as fallback text)
                geohashViewModel.sendGeohashMessage(photoFallbackText, selectedLocationChannel.channel, meshService.myPeerID, state.getNicknameValue())
            } else {
                // Send public/channel photo message via mesh
                if (currentChannelValue != null) {
                    channelManager.addChannelMessage(currentChannelValue, photoMessage, meshService.myPeerID)
                    // For encrypted channels, send as regular text fallback
                    if (channelManager.hasChannelKey(currentChannelValue)) {
                        meshService.sendMessage(photoFallbackText, emptyList(), currentChannelValue)
                    } else {
                        meshService.sendMessage(photoFallbackText, emptyList(), currentChannelValue)
                    }
                } else {
                    messageManager.addMessage(photoMessage)
                    meshService.sendMessage(photoFallbackText, emptyList(), null)
                }
            }
        }
    }
